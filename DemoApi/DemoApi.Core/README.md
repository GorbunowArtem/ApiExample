...
To update the database use this command from the Web project folder (replace `DemoApi.Core` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../DemoApi.Core.Infrastructure/DemoApi.Core.Infrastructure.csproj -s DemoApi.Core.Web.csproj
```
